...
**目前狀態（2026-05-10）**：M1-M3.5 已完成。Backend admin endpoints + frontend 7 頁全套可用、含 hotwords 匯入 / 匯出、TranscriptEditor auto-save、Dataset CRUD（list / import 4 格式 / export 3 格式 / from_job 一鍵建立 / 編輯）+ 4 範本下載。Editor 端 EditorSource 抽象 — Job / Dataset 共用同一 TranscriptEditor 元件。Backend 199 tests / ruff 0 / mypy 0；Frontend lint 0 errors / typecheck 0 errors。M4 / M5 / M6 / M7 屬下階段（Training / 長音檔切段 / v1 API / qc_simulator）。`git log` 看最新進度。
...
├── datasets                      M3.5：CRUD + 4 格式匯入 + 3 格式匯出 + from_job
│   ├── GET / POST / PUT / DELETE  /datasets[?project_id=][/:id]
│   ├── POST                       /datasets/import (multipart: audio + label + format)
│   ├── POST                       /datasets/from_job/:job_id  (從 SUCCEEDED Job 複製)
│   ├── GET                        /datasets/:id/audio  (FileResponse)
│   ├── GET                        /datasets/templates/{format}  (json/xlsx/srt/txt)
│   └── GET                        /datasets/:id/export?format=  (json/srt/xlsx)
└── system                        4 panel 狀態
    ├── GET  /system/health
...
```
frontend/src/
├── api/                  client.ts (fetch wrapper) + 各模組（含 datasets.ts）+ types.ts
├── pages/                Projects / Hotwords / Offline / Editor / Datasets /
│                         DatasetEditor / System (7 routes)
...
│                         ProjectFormModal / Toast / DatasetImportModal / FromJobModal
├── stores/               projectStore (全域) / editorStore (Editor scope) / toastStore (全域)
├── hooks/                useToast / useAutoSave / useKeyboardShortcuts
└── lib/                  format / time / keyboard / editorSource (Job/Dataset 共用 Editor 介面)
```
